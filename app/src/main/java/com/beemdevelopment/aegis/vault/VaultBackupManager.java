...
public class VaultBackupManager {
    ...
    private void createBackup(File tempFile)
        ...
        BackupVersioningStrategy strategy = _prefs.getBackupVersioningStrategy();
        if (strategy == null) {
            throw new VaultRepositoryException("getBackupVersioningStrategy returned null");
        }
        Uri uri = _prefs.getBackupsLocation();
        if (uri == null) {
            throw new VaultRepositoryException("getBackupsLocation returned null");
        }
        if (strategy == BackupVersioningStrategy.SINGLE_FILE) {
            createBackup(tempFile, uri);
        } else if (strategy == BackupVersioningStrategy.MULTIPLE_FILES) {
